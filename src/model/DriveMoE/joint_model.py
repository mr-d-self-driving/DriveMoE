...
from src.model.DriveMoE.mixture import Mixture, MoEMixture
from src.model.DriveMoE.mlp_router import MLPRouter


def forward_mixture_layers(
    mixtures: nn.ModuleList,
    attention_mask: torch.Tensor,
    position_ids_all: Dict[torch.LongTensor, torch.LongTensor],
    embeds_all: Dict[torch.FloatTensor, torch.FloatTensor],
    layer_idx: int,
    moe_start_layer: int,
    action_logits: torch.Tensor,
    action_router: nn.Module,
    post_attn_skip_names: List[str] = ["vlm"],
    kv_caches: Dict[KVCache, KVCache] = {},
    cache_mode: str = "append_non_active",
    time_cond: Optional[torch.FloatTensor] = None,
) -> Dict[torch.FloatTensor, torch.FloatTensor]:
    ...
    for name in active_mixture_names:
        if name in post_attn_skip_names:
            ...
            if name == "action" and layer_idx == moe_start_layer:
                # Update Action Logits
                assert action_logits is None, "Action Logits must be None!"
                action_logits = action_router(hidden_states_pre_mlp[name])
    ...
    return hidden_states_final, action_logits
...
def forward_mixture_attn(
    ...
    attention_mask: torch.Tensor,
    position_ids_all: Dict[torch.LongTensor, torch.LongTensor],
    hidden_states_all: Dict[torch.FloatTensor, torch.FloatTensor],
    layer_idx: int,
    post_attn_skip_names: List[str] = ["vlm"],
    kv_caches: Dict[KVCache, KVCache] = {},
    cache_mode: str = "append_non_active",
...
class JointModel(nn.Module):
    def __init__(self, config):
        ...
        for mixture_name, mixture_config in config.mixture.items():
            ...
            if mixture_name in ["vlm", "proprio"]:
                self.mixtures[mixture_name] = Mixture(mixture_config)
            else:
                self.mixtures[mixture_name] = MoEMixture(mixture_config)
        self.mixture_names = list(config.mixture.keys())
        self.action_router = MLPRouter()
        self.moe_start_layer = self.num_hidden_layers - config.num_moe_layers - 1
    ...
    def forward(
        ...
        for layer_idx in range(self.num_hidden_layers):
            ...
            embeds_all, action_logits = forward_mixture_layers(
                self.mixtures,
                attention_mask,
                position_ids_all,
                embeds_all,
        ...
        if return_caches:
            return hidden_states_all, action_logits, kv_caches
        return hidden_states_all, action_logits


if __name__ == "__main__":
    from omegaconf import OmegaConf

    cfg = OmegaConf.load("config/train/DriveMoE/stage1_closed_loop.yaml")
    model = JointModel(cfg.joint.config)

    # dummy inputs
    dummy_num_image_tokens = 7
    # dummy_action_logits = torch.randn(1, 7)
    dummy_action_logits = None
    q_lens = [
    ...
    for _step in range(num_step):
        ...
        embeds, action_logits = model(
            ...
            action_logits=dummy_action_logits,
            kv_caches=kv_caches,
            time_cond=time_cond,
